...
$related_videos    = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title,tags) AGAINST ('$query_video_title') AND id <> '{$get_video->id}' $not_in_query AND is_movie = 0 AND privacy = 0 ORDER BY `id` DESC LIMIT 20");
...
if (empty($related_videos)) {
    ...
    $db->where('privacy', 0);
    $related_videos = $db->where('category_id', $get_video->category_id)->where('is_movie',0)->where('id', $get_video->id, '<>')->get(T_VIDEOS, 20);
}
...
if (empty($related_videos)) {
    ...
    $db->where('privacy', 0);
    $related_videos = $db->where('id', $randomlySelected, 'IN')->where('is_movie',0)->where('id', $get_video->id, '<>')->get(T_VIDEOS);
}
...
if ($pt->is_wl === true && IS_LOGGED === true) {
    ...
    $wl_list_videos   = $db->where('id', array_values($video_list), 'IN')->orderBy('id','asc',array_values($video_list))->get(T_VIDEOS);
    $vid_number       = 1;
