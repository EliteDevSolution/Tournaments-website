...
if (!empty($_GET['first']) && $_GET['first'] == 'purchase') 
{
	if (!empty($_POST['type']) && $_POST['type'] == 'pro') 
	{
		...
		{
			if ($pt->config->recurring_payment == 'on' || $pt->config->recurring_payment == 'off') 
			{
				$redirectUrls->setReturnUrl(PT_Link('aj/go_pro/get_paid?status=success&pkg=pro'))->setCancelUrl(PT_Link(''));    
			    $item->setName('Purchase pro package')->setQuantity(1)->setPrice($sum)->setCurrency($payment_currency);  
			    $itemList->setItems(array($item));    
                ...
                $post_data['order_id'] = "proupgrade_".time();
                $post_data['bill_amount'] = $sum;
                $post_data['success_url'] = PT_Link('aj/go_pro/get_paid?status=success&pkg=pro');
...
if (!empty($_GET['first']) && $_GET['first'] == 'get_paid') 
{
	...
	if($pt->config->payment_mode == 'paypal')
	{
		if ($pt->config->recurring_payment == 'off') {
	...
	if (!empty($_POST['video_id']) && is_numeric($_POST['video_id'])) {
		...
		if (!empty($video)) {
			if($pt->config->payment_mode == 'fastpay')
			{
                ...
                $post_data['merchant_mobile_no'] = $pt->config->fastpay_id;
                $post_data['store_password'] = $pt->config->fastpay_secret;
                $post_data['order_id'] = "video_".$_POST['video_id'];
                $post_data['bill_amount'] = $video->sell_video;
                $post_data['success_url'] = PT_Link('aj/go_pro/paid_to_see?status=success&video_id='.$video->id);
                $post_data['fail_url'] = PT_Link('');
                $post_data['cancel_url'] = PT_Link('');
               	try
               	{
